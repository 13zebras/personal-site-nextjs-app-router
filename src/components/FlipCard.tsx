...
export default function FlipCard({ index, ...project }: Project) {
  ...
  return (
    <div id={`flipCard-${index}`} style={randomStyle} className='relative w-[260px] h-[230px] md:w-[210px] md:h-[220px] flipCard'>
      <div id={`flipContainer-${index}`} className='group z-20 [perspective:1200px] w-full h-full'>
        <div id='flipContent' className='w-full h-full transition-transform duration-500 ease-out delay-100 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]'>
          <div id='flipFront' className='absolute w-full h-full [backface-visibility:hidden] border-2 border-neutral-600 rounded-lg bg-zinc-920'>
            <div className='relative w-full h-full p-4 sm:px-3 flex flex-col items-center justify-start gap-y-3 md:gap-y-4'>
              <AdvancedImage id='flipImage' cldImg={frontImage} plugins={[placeholder({ mode: 'blur' })]} className='border border-zinc-800 rounded-md' />
              <div className='w-full h-[50px] min-h-[50px] px-8 sm:px-4 flex justify-center items-center text-center text-base leading-snug font-semibold text-zinc-200'>{project.name}</div>
            </div>
          </div>
          <div id='flipBack' className='absolute w-full h-full bg-slate-920 border-2 border-slate-600 rounded-lg p-6 sm:px-[16px] sm:py-[14px] flex flex-col items-center justify-start text-left text-md sm:text-sm sm:leading-snug text-zinc-300 [transform:rotateY(180deg)] [backface-visibility:hidden]'>
            ...
            {/* {showButton && ( */}
            <Button type='button' onClick={handleOpenClick} className='fixed bottom-2 w-40 h-6 border-zinc-600 text-zinc-350 text-[12px] tracking-wide0'>
              Learn More...
            </Button>
            {/* )} */}
